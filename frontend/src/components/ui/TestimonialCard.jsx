...
function TestimonialCard({ review }) {
  // Prefer real review fields, fallback to dummy review fields
  const client = review.userName || review.clientName || review.author;
  const trainer = (review.trainer && review.trainer.name) || review.trainerName || 'Our Trainer';
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-lg dark:shadow-black/20 p-8 hover:shadow-xl transition-all duration-300 border border-gray-100 dark:border-gray-700 h-full flex flex-col">
      ...
      <div className="pt-4 border-t border-gray-100 dark:border-gray-700">
        <div className="flex items-center justify-between mb-2">
          <p className="font-bold text-[#1a1a1a] dark:text-white text-lg">{client}</p>
          {/* Stars */}
        ...
        <p className="text-sm text-gray-500">
          With trainer <span className="font-semibold text-[#775fab]">{trainer}</span>
        </p>
      </div>
    </div>
  );
